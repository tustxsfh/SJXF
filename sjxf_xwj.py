...
articleList = json.loads(requests.get("http://221.204.170.88:8184/app/study/list_article/72?size=10&page=1").text)["data"]
# print(articleList)

#登录
def login(username, password):
    url = 'http://221.204.170.88:8184/app/user/login'
    data = {"clientid": "234234", "deviceId": "id98fa4c9d-4db3-40e7-82e3-17218d29ec0e", "password": password,
            "userName": username, }
    headers = {
...
def do(Authorization, userId):
    def get_appStartTime():
        url = "http://221.204.170.88:8184/app/time"
        ...
        Rsponse = requests.get(url, headers=headers)
        app_start_time = json.loads(Rsponse.text)["data"]
    ...
    def read():
        ...
        read_Rsponse = requests.post(url, headers=headers, json=payload)
        # print("阅读文章====================" + read_Rsponse.text)
    # read()
    # read()
 ...
 # 积分明细
    def jifen():
        ...
        if str(todayScore['data']['todayScore']) < '15.0':
            global desp
            desp = '【分值不够，请手动刷分!】薛文杰今天增加了' + str(todayScore['data']['todayScore']) + '分，总分为' + str(
                todayScore['data']['yearScore']) + '分'
            print(desp)
            # rizhi()
        else:
            desp = '薛文杰今天增加了' + str(todayScore['data']['todayScore']) + '分，总分为' + str(
                todayScore['data']['yearScore']) + '分'
            print(desp)
...
username='15903500868'                         #薛文杰的账号
password ='c0Ps362i42AHqSsdJmiFlQ=='           #薛文杰的密码
