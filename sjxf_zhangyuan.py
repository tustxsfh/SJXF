 ...
 # 积分明细
    def jifen():
        ...
        if str(todayScore['data']['todayScore']) < '15.0':
            global desp
            desp = '【分值不够，请手动刷分!】张院长今天增加了' + str(todayScore['data']['todayScore']) + '分，总分为' + str(
                todayScore['data']['yearScore']) + '分'
            print(desp)
            # rizhi()
        else:
            desp = '张院长今天增加了' + str(todayScore['data']['todayScore']) + '分，总分为' + str(
                todayScore['data']['yearScore']) + '分'
            print(desp)
...
username='13935076918'                         #张院长的账号
password ='v1Vh8Aq8V8Rg/UOYBitkmg=='           #张院长的密码
