 ...
 # 积分明细
    def jifen():
        ...
        if str(todayScore['data']['todayScore']) < '15.0':
            global desp
            desp = '【分值不够，请手动刷分!】季蓉今天增加了' + str(todayScore['data']['todayScore']) + '分，总分为' + str(
                todayScore['data']['yearScore']) + '分'
            print(desp)
            # rizhi()
        else:
            desp = '季蓉今天增加了' + str(todayScore['data']['todayScore']) + '分，总分为' + str(
                todayScore['data']['yearScore']) + '分'
            print(desp)
...
username='15835660100'                         #季蓉的账号
password ='Nnu/6slPtYPwroSFGhJgjQ=='           #季蓉的密码
