 ...
 # 积分明细
    def jifen():
        ...
        if str(todayScore['data']['todayScore']) < '15.0':
            global desp
            desp = '【分值不够，请手动刷分!】胡正斌今天增加了' + str(todayScore['data']['todayScore']) + '分，总分为' + str(
                todayScore['data']['yearScore']) + '分'
            print(desp)
            # rizhi()
        else:
            desp = '胡正斌今天增加了' + str(todayScore['data']['todayScore']) + '分，总分为' + str(
                todayScore['data']['yearScore']) + '分'
            print(desp)
...
username='15835660063'                         #胡正斌的账号
password ='ROkB7zglzHL3xYPn3SKH4g=='           #胡正斌的密码
