 ...
 # 积分明细
    def jifen():
        ...
        if str(todayScore['data']['todayScore']) < '15.0':
            global desp
            desp = '【分值不够，请手动刷分!】彭素军今天增加了' + str(todayScore['data']['todayScore']) + '分，总分为' + str(
                todayScore['data']['yearScore']) + '分'
            print(desp)
            # rizhi()
        else:
            desp = '彭素军今天增加了' + str(todayScore['data']['todayScore']) + '分，总分为' + str(
                todayScore['data']['yearScore']) + '分'
            print(desp)
...
username='13603502808'                         #彭素军的账号
password ='Y0PSjIY/6D6L6FuQydijUw=='           #彭素军的密码
